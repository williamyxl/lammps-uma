#!/usr/bin/env python3
"""uma/kk — first-frame E+F + NVT @ 300 K Pair timing (default 10 steps)."""
...
def main() -> int:
    ...
    ap.add_argument(
        "--nsteps",
        type=int,
        default=int(os.environ.get("NSTEPS", str(DEFAULT_NSTEPS))),
    )
    ...
    data = work / "nacl6_nvt_300K_atomic_metal.data"
    shutil.copy2(args.data, data)
    dump_sp = work / "forces_first.dump"
    log = work / "log.uma"

    # type1=Na type2=Cl (see prep_nacl6_nvt_data.py / Masses)
...
# (2) timed SP proxy: 1 NVE step → Pair ms
fix _sp all nve
run 1
unfix _sp

# (3) timed NVT @ 300 K (no dumps)
...
"""
    ...
    lmp = find_uma_lmp()
    env = setup_ld_path(natoms=NATOMS, uma_devices=uma_devices)
    ...
    if uma_devices > 1:
        ...
        shard = art / f"model_mp_w{uma_devices}_n{NATOMS}_r0.pt"
        if not shard.is_file():
            raise FileNotFoundError(
                f"missing {shard} — run export_mp_nacl6 / export_mp_w{uma_devices}"
            )
    cmd = [str(lmp)]
    if use_kk:
    ...
    write_timing(
        out,
        {
            ...
            "note": (
                ...
                "Default NSTEPS=10 (water888 campaign often uses 100)."
            ),
        },
        forces=f,
    )
