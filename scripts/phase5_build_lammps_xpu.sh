#!/bin/bash -l
# Build LAMMPS (no Kokkos) with ML-UMA on Intel XPU (eager engine path).
# pair_style uma (non-kk) + uma-engine XPU build. GCC 13.4 + Cray MPICH +
# torch+xpu libtorch, forcing conda libsycl.so.9 over system .8.
set -e
