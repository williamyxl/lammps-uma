...
| 2026-09-01 | note | XPU rebuild+G4 revalidation of R2/R4 pending: repeated build jobs (8794013/8794060/8794073) **stalled in activation** (cput frozen ~15 s, shared-FS contention), not a code fault. R1 clean-clone build + R2 standalone-configure + R3 Tier-1 + R4 DD-only-scope give static/partial verification; the full tripwire+G4 will run once the build node clears. | 8794073 (queued) |
